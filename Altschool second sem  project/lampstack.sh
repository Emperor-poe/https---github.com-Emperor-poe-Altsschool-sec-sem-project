...
vagrant ssh master <<EOF
    ...
    sudo cat /home/vagrant/.ssh/id_rsa.pub | sshpass -p "vagrant" ssh -o StrictHostKeyChecking=no vagrant@192.168.10.3 'mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys'
    sudo cat ~/altschoolkey | sshpass -p "vagrant" ssh vagrant@192.168.10.3 'mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys'
    sshpass -p "pelumi" sudo -u altschool mkdir -p /mnt/altschool/slave
    sshpass -p "pelumi" sudo -u altschool scp -r /mnt/* vagrant@192.168.10.3:/home/vagrant/mnt
